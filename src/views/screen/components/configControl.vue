    ...
    <div class="main-right" :class="{ lock: isLockDisplay }">
        <el-tabs class="tabs" model-value="position">
            <el-tab-pane label="属性配置" name="position">
                <el-form size="small">
                    <el-form-item label="版本筛选" v-if="current.optionversion">
                        <el-select v-model="currentOptionVersion" @change="optionVersionChange">
                            <el-tooltip :content="item.versiondescribe" v-for="(item, idx) in formatVersionCfg(current.optionversion)" :key="idx">
                    ...
                    <el-form-item :label="item.label" v-for="(item, index) in positionProperty" :key="index">
                        ...
                        />
                        <el-input v-else :placeholder="item.placeholder" :model-value="item.value" @input="propertyChange(item, $event)" />
                    </el-form-item>
                    ...
                    <el-form-item v-if="['text', 'sText', 'image', 'background', 'border', 'clock'].includes(type as string)">
                ...
                <el-form size="small" v-if="['line', 'bar', 'pie', 'echarts', 'text', 'sText', 'table'].includes(type as string)">
                    <el-form-item label="版本">
                        <el-input
                            v-model="current.config.version"
                            type="text"
                            placeholder="请输入版本"
                    ...
                    <el-form-item label="版本描述">
                        <el-input
                            v-model="current.config.versiondescribe"
                            type="text"
                            placeholder="请输入版本描述"
                    ...
                    <el-form-item label="是否发布">
                        <el-radio-group v-model="current.config.ispublish" @change="e => propertyChange({ key: 'ispublish' }, e)">
                    ...
                    <template v-if="current.config.optionsType === 1">
                        <el-form-item label="插件标识">
                            <div style="display: flex">
                                ...
                                <el-button type="primary" @click="dialogShow = true" style="margin-left: 5px">数据配置</el-button>
                            </div>
                        </el-form-item>
                    </template>
                    <template v-if="current.id">
                        <el-form-item>
                            ...
                            <el-button type="primary" @click="refreshChartoption">刷新</el-button>
                        </el-form-item>
                    </template>
                </el-form>
            </el-tab-pane>
                ...
                <el-form size="small" :rules="[]">
                    <el-form-item label="版本筛选" v-if="config.configversion">
                        <el-select v-model="currentConfigVersion" @change="configVersionChange">
                            <el-tooltip :content="item.versiondescribe" v-for="(item, idx) in formatVersionCfg(config.configversion)" :key="idx">
                                <el-option :value="item.version" :label="item.versionLabel" />
                            </el-tooltip>
                        </el-select>
                    </el-form-item>
                    ...
                    <el-form-item label="大屏标识">
                        <div style="display: flex; align-items: center">
                            <el-input placeholder="请输入大屏标识" :model-value="config.configtag" @input="configChange('configtag', $event)" />
                            <el-button type="primary" @click="screenSearch">查询</el-button>
                        </div>
                    </el-form-item>
                    <el-form-item label="版本">
                        <el-input placeholder="请输入版本" :model-value="config.version" @input="configChange('version', $event)" />
                    </el-form-item>
                    <el-form-item label="版本表述">
                        <el-input
                            placeholder="请输入版本表述"
                            :model-value="config.versiondescribe"
                    ...
                    <el-form-item label="大屏高度">
                        <el-input placeholder="请输入大屏高度" :model-value="config.height" @input="configChange('height', $event)" />
                    </el-form-item>
...
<script setup lang="ts">
    ...
    const saveChartoption = () => {
        ...
        data.configparentid = props.config.id;
        KDModel.invoke("saveoption", JSON.stringify(data));
    };
    ...
    const refreshChartoption = () => {
        const data = { ...current.value };
        data.configparentid = props.config.id;
        KDModel.invoke("refresh", JSON.stringify(data));
    };

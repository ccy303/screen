<template>
    <div ref="designBoxEl" class="design-box" @scroll="designScroll">
        <a-ruler :scale="state.scale" :scroll="state.scroll" :show-ruler="state.ruler" size="1920px" />
        <a-ruler :scale="state.scale" :scroll="state.scroll" :show-ruler="state.ruler" direction="v" size="1080px" />
        <div :class="{ preview: preview }" :style="canvasStyle" class="design-canvas" @contextmenu="contextmenu"
            @mousedown.left="onmousedown">
            <draggable v-bind="{
        group: 'form',
        animation: 200,
        handle: '.none'
    }" :list="data.list" class="drag" itemKey="id" name="fade" @add="draggableAdd">
                <template #item="{ element, index }">
                    <ak-screen v-if="getScreenShowHide(element)" :current="state.activeIndex === index"
                        :data="getReplaceGlobal(element)" :scale="state.scale" :type="0"
                        @contextmenu="contextmenuScreen" @change-event="changeEvent(index, $event, state.tempRectIndex)"
                        @click.stop="itemClick(element, index)" />
                </template>
            </draggable>
            <div :style="state.rect" class="draw-react"></div>
        </div>
        <div v-if="!data.list.length" class="no-date">请从左则组件栏拖动组件到设计区域</div>
    </div>
    <div class="design-footer">
        ...
        <div class="center">
            ...
            <div class="item slider">
                ...
                <el-slider v-model="state.scale" :marks="marks" :max="200" :min="state.autoScale - 30" show-stops
                    size="small" @change="sliderChange" />
            </div>
            <div class="item">
                <el-button link type="primary" @click="defaultScaleClick('auto')">自适应 </el-button>
                <el-button link type="primary" @click="defaultScaleClick('100')">100% </el-button>
            </div>
        </div>
...
import { addUnit, cannotDragResize, canRectSelect, removeUnit } from "../utils";
import type { ScreenData, UpdatePosition, Contextmenu2, Contextmenu } from "../types";
...
const props = withDefaults(
    defineProps<{
        preview: boolean;
        data: any;
        updatePosition: (index: number, data: UpdatePosition, tempRectIndex: number[]) => void;
...
const findComponentsInRect = () => {
    ...
    props.data.list.forEach((item: ScreenData, index: number) => {
        ...
        if (!cannotDragResize(item.position) && isRectOverlap(item.position) && canRectSelect(item)) {
...
const getInit = () => {
    nextTick(() => {
        ...
        state.offset[1] = componentRect.top + document.documentElement.scrollTop - (document.documentElement.clientTop || 0);
        state.offset[0] = componentRect.left + document.documentElement.scrollLeft - (document.documentElement.clientLeft || 0);
    });
};

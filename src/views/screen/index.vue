...
<template>
    <div v-loading="loading" class="container-screen">
        ...
        <control-left ref="controlLeftEl" :style="{ width: toolVisible('left') ? '' : '0px' }" @update="controlEvent" />
        <div class="main-box">
            <head-tools @click="headToolsClick" />
            <screen-design :preview="preview" :data="screenData" :update-position="updatePosition"
                :screenContextmenu="screenContextmenu" @draggable-add="setLayerList" />
        </div>
        <config-control ref="configEl" v-model:config="screenData.config"
            :style="{ width: toolVisible('right') ? '' : '0px' }" @update="setLayerList" @openDrawer="openDrawer" />
        <ace-drawer v-model="drawer.visible" :code-type="drawer.codeType" :content="drawer.content"
            :direction="drawer.direction" :title="drawer.title" @confirm="dialogConfirm"
 ...
 */
const updatePosition = (index: number, obj: UpdatePosition, tempRectIndex: number[]) => {
    if (obj.type === "move") {
        // 移动时
        ...
        if (list.type === "tempRect") {
            ...
            tempRectIndex.forEach((index: number) => {
                ...
                tempIndex.position.left = parseInt(tempIndex.position.left) + obj.moveX;
...
const rightMenuClick = (key: string, data: any) => {
    ...
    switch (key) {
        ...
        case "verticalCenter":
            ...
            for (const key in data.tempRectIndex) {
                const obj = list[data.tempRectIndex[key]];
                obj.position[leftTop] = center - removeUnit(obj.position[widthHeight]) / 2;

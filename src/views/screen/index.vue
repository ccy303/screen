...
<script lang="ts" setup>
    ...
    const rightMenuClick = (key: string, data: any) => {
        ...
        switch (key) {
            ...
            case "del":
                ...
                [delData] = screenData.value.list.splice(data.activeIndex, 1);
                delOptionIds.value.push(delData?.id);

                if (isGroupId && currentData.type === "group") {
                    screenData.value.list = list.filter((item: ScreenData) => {
                        if (item.groupId == isGroupId) {
                            item.id && delOptionIds.value.push(item.id);
                        }
                        return item.groupId !== isGroupId;
                    });
                }
                if (type === "tempRect") {
                    for (let i = data.tempRectIndex.length - 1; i >= 0; i--) {
                        [delData] = screenData.value.list.splice(data.tempRectIndex[i], 1);
                        delOptionIds.value.push(delData?.id);
                    }
                }
    ...
    //****************数据相关****************//
    const saveData = () => {
        const data: any = { ...screenData.value, config: { ...screenData.value.config, deloptionids: (delOptionIds.value || []).join(",") } };
